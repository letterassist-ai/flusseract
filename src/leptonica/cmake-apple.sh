...
if [[ ' iphoneos iphonesimulator ' =~ $APPLE_SDK ]]; then
  export CFLAGS="-Wno-unused-variable -Wno-empty-body -Wno-literal-range -arch $ARCHITECTURE -O3 -miphoneos-version-min=11.0 -funwind-tables"
else
  export CFLAGS="-Wno-unused-variable -Wno-empty-body -Wno-literal-range -arch $ARCHITECTURE -O3 -mmacosx-version-min=10.13"
fi
